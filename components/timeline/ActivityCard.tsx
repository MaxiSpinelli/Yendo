...
export default function ActivityCard({ activity, onRefresh }: ActivityCardProps) {
  ...
  return (
    <>
      <div className="card p-4 border-l-4 border-l-brand-400 hover:shadow-md transition-shadow">
        <div className="flex items-start justify-between gap-2">
          <div className="flex items-center gap-2.5 min-w-0">
            ...
            <div className="min-w-0">
              <p className="font-semibold text-stone-900 text-sm truncate">{activity.name}</p>
              {activity.location && (
                <p className="text-xs text-stone-500 mt-0.5 truncate">{activity.location}</p>
              )}
            </div>
          </div>
